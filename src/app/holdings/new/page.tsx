...
export default function HoldingsPage() {
  ...
  // Calculate totals
  const totalPurchaseJPY = holdings?.reduce((sum: number, h: any) => {
    const local = h.quantity * h.avg_purchase_price;
    return sum + (isUSD(h.purchase_currency) && usdJpy ? local * usdJpy : local);
  }, 0) || 0;

  const totalLiveJPY = priceUpdated
    ? Object.values(livePrices).reduce((sum, p) => sum + p.liveValueJPY, 0)
    : 0;

  const totalDiff = priceUpdated ? totalLiveJPY - totalPurchaseJPY : 0;
  ...
  return (
    <div>
      ...
      {/* Total summary when prices are updated */}
      {priceUpdated && (
        <Card className="mb-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500">Total Cost</p>
              <p className="text-lg font-bold text-gray-700">Y{Math.round(totalPurchaseJPY).toLocaleString()}</p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-500">Current Value</p>
              <p className="text-lg font-bold text-brand-700">Y{Math.round(totalLiveJPY).toLocaleString()}</p>
            </div>
                      ...
                      <div>
                        ...
                        {price && (
                          <div className="mt-2 p-2 bg-gray-50 rounded border border-gray-100">
                            <p className="text-xs text-gray-400">
                              Current Price: {price.liveCurrency === 'USD' ? '$' : 'Y'}
                              {price.livePrice.toLocaleString()}
                            </p>
                            <p className="font-bold text-brand-700">
                              Current Value: Y{price.liveValueJPY.toLocaleString()}
                            </p>
                        ...
                        {!priceUpdated && (
                          <p className="text-xs text-gray-300 mt-1">
                            Click &quot;Update Prices&quot; for live data
                          </p>
                        )}
                      </div>
                    )}
